...
def add_title_slide(prs: Presentation) -> None:
    ...
    textbox(
        ...
        [
            (
                "Coffee pages for creators. Payment links for everyone.",
                20,
                False,
                MUTED,
            ),
...
def add_two_product_slide(prs: Presentation) -> None:
    ...
    add_card(
        ...
        3.35,
        "☕ Coffee page",
        "Buy-me-a-coffee style page. Fans pick 1, 3, or 5 coffees, leave a note, and pay in one tap from any chain.",
    )
    add_card(
        slide,
        6.75,
...
def add_demo_slide(prs: Presentation) -> None:
    ...
    steps = [
        "Log in with email on dogi.app (or localhost)",
        "Create a coffee page or payment link on the dashboard",
        "Open the link in incognito with a second email",
        "Pay with any supported token on any chain",
        "Receiver balance updates as USDC on Arbitrum",
    ]
    y = 2.0
...
def build() -> Path:
    ...
    add_bullet_slide(
        prs,
        "PROBLEM",
        "Getting paid across chains is still too hard",
        [
            "Creators and freelancers need wallet apps, seed phrases, and chain knowledge",
            "Payers hold assets on different chains and tokens",
            "Bridging, swapping, and gas kill conversion on simple payments",
            "Buy Me a Coffee works for fiat. Nothing comparable exists for Web3 natives",
        ],
        "Anchor on a real creator or freelancer who just wants a link, not a lecture on L2s.",
    )
    add_bullet_slide(
        prs,
        "SOLUTION",
        "Dogi: pay links that feel like email checkout",
        [
            "Payers log in with email and pay with whatever they hold",
            "Recipients always receive USDC on Arbitrum",
            "No wallet app, no seed phrase, no visible bridging",
            "Fully client-side: payment data lives in the URL, no backend",
        ],
        "This is the 30-second pitch. Everything else supports this claim.",
    )
