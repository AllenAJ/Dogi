...
Dogi turns cross-chain payments into something anyone can do. Two products share the
same rails:

1. **Treat page**: a Buy-me-a-coffee-style page for creators. Fans pick 1, 3, or 5
   treats, add a note, log in with email, and pay in one tap.
2. **Payment links**: invoices and one-off requests ("$25 for Invoice #42") shared as
   a link.
...
| `src/app/dashboard/page.tsx` | Unified balance + treat page setup + payment links |
| `src/app/tip/[code]/page.tsx` | Public creator page: send 1/3/5 treats cross-chain |
| `src/app/pay/[code]/page.tsx` | The pay page: log in with email → one-tap cross-chain payment |
| `src/lib/links.ts`, `src/lib/creator.ts` | Links and creator pages are self-contained; the payload is encoded in the URL, no backend |
...
`NEXT_PUBLIC_DEMO_MODE=1` simulates balances, routes, payments, and the received
feed (login stays real) so the full flow can be recorded without funded accounts.
A fixed "Simulated demo" badge is shown whenever it's on. Local use only; never
...
3. On the dashboard, create your treat page (name, avatar, price) and/or a payment
   link. Both give you a shareable URL.
4. Open the URL (incognito or another device), log in with a *different* email, and
   send a few treats or pay the invoice.
